...
export function ouvirTarefasDeUsuario(pessoaId: string, cb: (tarefas: Tarefa[]) => void): Unsubscribe {
  ...
  // "Minhas" = tarefas ATRIBUÍDAS a mim. NÃO inclui tarefas que eu só criei e
  // atribuí a outra pessoa (essas são "dela" — apareço nelas pela aba Tudo, que
  // já libera pelo criadoPor via podeVerTarefa). Antes o listener de `criadoPor`
  // fazia a tarefa do outro aparecer em Minhas.
