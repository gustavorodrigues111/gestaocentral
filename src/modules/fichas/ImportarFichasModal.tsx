                    ...
                    <div key={p.key}>
                      <div className="space-y-1">
                        ...
                        <div className="flex items-center gap-1.5 flex-wrap">
                          <button type="button" onClick={() => abrirPick(p.key)} title="Vincular a outro insumo / juntar com outro do lote" className="h-8 w-8 flex items-center justify-center rounded-full bg-gray-100 dark:bg-gray-800 text-gray-500 hover:text-indigo-600 shrink-0">🔎</button>
                          <select value={p.matchInsumoId ?? "__novo__"} onChange={e => { const val = e.target.value; setPrinc(p.key, val === "__novo__" ? { matchInsumoId: null, status: "novo" } : { matchInsumoId: val, status: "casado" }); }} className="h-8 flex-1 min-w-[150px] text-xs px-3 rounded-full bg-gray-100 dark:bg-gray-800 border-0 text-gray-700 dark:text-gray-200 cursor-pointer">
