                  ...
                  {principaisLista.map(p => (
                    <div key={p.key}>
                      <div className="space-y-1">
                        ...
                        {/* Linha 2: vínculo (nome visível) + reclassificações */}
                        <div className="flex items-center gap-1.5 flex-wrap">
                          <button type="button" onClick={() => abrirPick(p.key)} title="Vincular a outro insumo / juntar com outro do lote" className="text-xs px-1.5 py-1.5 rounded-lg border border-gray-300 dark:border-gray-700 text-gray-500 hover:text-indigo-600 hover:border-indigo-400 shrink-0">🔎</button>
                          <select value={p.matchInsumoId ?? "__novo__"} onChange={e => { const val = e.target.value; setPrinc(p.key, val === "__novo__" ? { matchInsumoId: null, status: "novo" } : { matchInsumoId: val, status: "casado" }); }} className="flex-1 min-w-[150px] text-xs px-2 py-1.5 rounded-lg border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-900">
