...
export function ImportarFichasModal({ rid, insumos, categorias, fichasExistentes, meId, meNome, onClose }: {
  ...
  function promoverParaSubficha(pk: string, vNorm?: string, targetId?: string) {
    ...
    <div key={f.id} className={`rounded-xl border overflow-hidden ${f.incluir ? "border-gray-200 dark:border-gray-800" : "border-gray-200 dark:border-gray-800 opacity-50"}`}>
      <div className="p-2.5 bg-gray-50 dark:bg-gray-800/40 border-b border-gray-100 dark:border-gray-800 space-y-1.5">
        ...
        <div className="flex items-center gap-1.5 flex-wrap pl-6">
          ...
          <select value={f.categoriaId || ""} onChange={e => setFicha(f.id, { categoriaId: e.target.value || null })} className="h-8 px-3 text-xs rounded-full bg-gray-100 dark:bg-gray-800 border-0 text-gray-600 dark:text-gray-300 shrink-0 max-w-[140px] cursor-pointer"><option value="">sem categoria</option>{catsAtivas.filter(c => (c.tipo || "ficha") === (f.ehSubficha ? "subficha" : "ficha")).sort((a, b) => a.nome.localeCompare(b.nome)).map(c => <option key={c.id} value={c.id}>{UP(c.nome)}</option>)}</select>
          {f.ehSubficha && (
          ...
          <div ref={scrollRef} className="max-h-[55vh] overflow-y-auto pr-1 space-y-4">
            ...
            {passo === 1 && !pickAlvo && (
              <div className="rounded-xl border border-gray-200 dark:border-gray-800">
                ...
                {cont.conferir > 0 && <div className="px-3 py-1.5 text-[11px] text-amber-700 dark:text-amber-300 bg-amber-50/60 dark:bg-amber-900/10 border-b border-gray-100 dark:border-gray-800"><strong>reconhecido</strong> = casou certo · <strong>novo</strong> = vai criar · <strong>conferir</strong> = palpite; se estiver certo, clique no selo <strong>“conferir ✓”</strong> pra confirmar (ou troque o insumo).</div>}
                <div className="p-2 space-y-2">
                  {principaisLista.map(p => (
                    <div key={p.key}>
                      <div className="space-y-1">
                        {/* Linha 1: nome + uso + status */}
                        <div className="flex items-center gap-2 text-sm">
                          <input value={p.nome} onChange={e => setPrinc(p.key, { nome: e.target.value.toUpperCase() })} className="flex-1 min-w-0 bg-transparent font-medium text-gray-800 dark:text-gray-200 outline-none border-b border-dashed border-gray-300 dark:border-gray-600 focus:border-solid focus:border-indigo-500 px-0.5" />
                          <span className="text-[11px] text-gray-400 shrink-0 cursor-help underline decoration-dotted underline-offset-2" title={`Usado em: ${(preparosPorPrincipal[p.key] || []).join(", ") || "—"}`}>{usoPrincipal[p.key]} uso(s)</span>
                          {p.status === "conferir" && p.matchInsumoId
                            ? <button type="button" onClick={() => setPrinc(p.key, { status: "casado" })} title="Confirmar que é este insumo → vira reconhecido" className={`text-[10px] font-medium px-2 py-0.5 rounded-full shrink-0 ${CHIP.conferir} hover:ring-1 hover:ring-amber-400`}>conferir ✓</button>
                        ...
                        {/* Linha 2: vínculo (nome visível) + reclassificações — chips cinza h-8 */}
                        <div className="flex items-center gap-1.5 flex-wrap">
                          <select value={p.matchInsumoId ?? "__novo__"} onChange={e => { const val = e.target.value; if (val === "__pesquisar__") { abrirPick(p.key); return; } setPrinc(p.key, val === "__novo__" ? { matchInsumoId: null, status: "novo" } : { matchInsumoId: val, status: "casado" }); }} className="h-8 flex-1 min-w-[180px] text-xs px-3 rounded-full bg-gray-100 dark:bg-gray-800 border-0 text-gray-700 dark:text-gray-200 cursor-pointer">
              ...
              <div className="space-y-3">
                ...
                {subfichas.length > 0 && (
                  <div className="flex items-center gap-2 flex-wrap text-xs">
                    ...
                    <select onChange={e => catTodas(e.target.value, true)} className="text-xs px-2 py-1 rounded border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-900"><option value="">— escolher —</option>{catsAtivas.filter(c => (c.tipo || "ficha") === "subficha").sort((a, b) => a.nome.localeCompare(b.nome)).map(c => <option key={c.id} value={c.id}>{UP(c.nome)}</option>)}</select>
                  </div>
                )}
            ...
            {passo === 3 && (
              <div className="space-y-3">
                <div className="flex items-center gap-2 flex-wrap text-xs">
                  ...
                  <select onChange={e => catTodas(e.target.value, false)} className="text-xs px-2 py-1 rounded border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-900"><option value="">— escolher —</option>{catsAtivas.filter(c => (c.tipo || "ficha") === "ficha").sort((a, b) => a.nome.localeCompare(b.nome)).map(c => <option key={c.id} value={c.id}>{UP(c.nome)}</option>)}</select>
                </div>
