...
function NovoFechamentoModal({ rid, restaurant, pessoas, por, recentes, onClose, onSalvo }: {
  ...
  async function lerTotal(f: File) {
    ...
    try {
      ...
      if (resp.ok) {
        ...
        if (j.turno === "almoco" || j.turno === "jantar" || j.turno === "dia") setTurno(j.turno);
  ...
  async function salvar() {
    ...
    try {
      ...
      // Canal do aviso aos sócios: default "ambos" (retrocompat — antes mandava os dois).
...
function FechamentoConfig({ rid, restaurant, pessoas }: { rid: string; pessoas: Pessoa[]; restaurant: { nome?: string; driveRootFolderId?: string; driveRootFolderNome?: string; fechamentoSociosPessoaIds?: string[]; fechamentoSociosEmails?: string[]; fechamentoSociosWhatsapp?: string[]; fechamentoCanalEnvio?: "email" | "whatsapp" | "ambos"; fechamentoEmailRemetente?: string; fechamentoComandas?: ComandaCadastro[]; fechamentoTemIfood?: boolean; fechamentoPedirObsTurno?: boolean } }) {
  ...
  return (
    <div className="space-y-4 max-w-2xl">
      ...
      <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl p-4 space-y-3">
        <h3 className="font-semibold text-gray-900 dark:text-gray-100">Aviso aos sócios</h3>
        {/* Canal do envio — controla o que dispara a cada fechamento */}
        <div>
          ...
          <p className="text-[11px] text-gray-400 mt-1">
            {canal === "email" ? "Só o email é disparado — os números de WhatsApp ficam salvos mas não recebem." : canal === "whatsapp" ? "Só o WhatsApp é disparado — os emails ficam salvos mas não recebem." : "Email e WhatsApp são disparados a cada fechamento."}
          </p>
        </div>
