// ════════════════════════════════════════════════════════════════════════════
//  Modal — Exportar VR em PDF
//
//  Mesmo modelo do VT: gera o PDF do lote/preview de Vale Refeição com a forma
//  de pagamento (Caju/PIX) por empregado, mostra preview e só baixa no clique.
// ════════════════════════════════════════════════════════════════════════════
...
export function ExportarVRModal({ ano, mes, restaurantNome, statusLabel, linhas, onClose }: Props) {
  ...
  useEffect(() => {
    ...
    async function gen() {
      ...
      setGerando(true);
      try {
        const doc = await gerarVRPDF({ ano, mes, restaurantNome, statusLabel, linhas });
        if (cancelled) return;
        docRef.current = doc;
  ...
  function baixar() {
    ...
    docRef.current.save(`vr-${slugify(restaurantNome)}-${ano}-${pad2(mes)}.pdf`);
  }
      ...
      <div className="space-y-4">
        ...
        <div className="rounded-lg border border-gray-300 dark:border-gray-700 bg-gray-100 dark:bg-gray-800 h-[55vh] flex items-center justify-center overflow-hidden">
          {gerando ? (
            ...
            <iframe src={previewUrl} title="Pré-visualização do VR" className="w-full h-full bg-white" />
          ) : null}
        </div>
