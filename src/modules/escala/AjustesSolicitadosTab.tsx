...
export function AjustesSolicitadosTab({ rid }: { rid: string }) {
  ...
  async function aprovar(s: EscalaSolicitacao) {
    ...
    try {
      ...
      if (esc?.fechadoEm && !window.confirm("O mês desse dia já foi FECHADO (praticada). Aplicar mesmo assim vai alterar o registro final — usado em gorjeta/VT/folha. Confirmar?")) { setProcessando(null); return; }
      if (s.gorjetaPaga && !window.confirm("A gorjeta desse dia já foi publicada. Aplicar o ajuste não recalcula automaticamente o que já foi pago. Confirmar mesmo assim?")) { setProcessando(null); return; }
          ...
          {pendentes.map((s) => (
            <div key={s.id} className="rounded-xl border border-amber-200 dark:border-amber-900/40 bg-amber-50/40 dark:bg-amber-950/10 p-3 space-y-2">
              <div className="flex items-start justify-between gap-2">
                <div>
                  <div className="text-sm font-semibold text-gray-900 dark:text-gray-100">{s.empregadoNome || "Empregado"} · {fmtDia(s.data)}</div>
                  <div className="text-[12px] text-gray-600 dark:text-gray-300 mt-0.5">
                    De <strong>{s.statusAtual ? LABEL[s.statusAtual] : "—"}</strong> → <strong className="text-indigo-700 dark:text-indigo-300">{LABEL[s.statusSolicitado]}</strong>
                  </div>
                </div>
              ...
              {s.gorjetaPaga && <p className="text-[11px] text-amber-700 dark:text-amber-300">⚠ Gorjeta desse dia já publicada.</p>}
              <div className="flex justify-end gap-2">
                <Button variant="secondary" size="sm" disabled={processando === s.id} onClick={() => void recusar(s)}>Recusar</Button>
                <Button size="sm" disabled={processando === s.id} onClick={() => void aprovar(s)}>{processando === s.id ? "…" : "Aprovar e aplicar"}</Button>
              </div>
            </div>
          ))}
        </div>
            ...
            <div className="mt-2 space-y-1.5">
              {respondidos.map((s) => (
                <div key={s.id} className="rounded-lg border border-gray-100 dark:border-gray-800 px-3 py-2 text-[12px] flex items-center justify-between gap-2">
                  <span className="text-gray-700 dark:text-gray-300">{s.empregadoNome} · {fmtDia(s.data)} · {LABEL[s.statusSolicitado]}</span>
