...
const SEED: Seed[] = [
  ...
  { modulo: "ptrp", resp: "gustavo", titulo: "PTRP × Escala praticada: só NOVOS abonos/atestados (pós-deploy 2cc6091) gravam escalas.real. Tratamentos ANTIGOS (só ptrpAjustes) não aparecem na praticada até re-salvar / reclassificar / 'Gerar praticada' (que hoje só aparece com o mês ENCERRADO). Decidir: backfill 1-clique no mês aberto?", descricao: "Wiring conferido OK (rid, doc id YYYY-MM, empId, dia, célula = real ?? prevista). O buraco é retroativo: quem tratou antes do deploy não retro-preencheu real. Opção: liberar 'Gerar praticada' (statusPraticado→real, sem solides_sync) no mês aberto, ou um 'sincronizar praticada' por colaborador." },
  { modulo: "ptrp", resp: "ia", titulo: "FEITO: Reorganizar batidas reabre na ordem SALVA (não re-ordena cronológico); Abono/Atestado de dia inteiro grava já em escalas.real (praticada) — antes só mudava ao 'Fechar praticada'; Conferência ganhou coluna Previsto × Praticado (badge + '•' quando difere).", descricao: "1) abrirReorganizar recebe reorgExistente e achata os pares salvos. 2) caminho 'motivo' (diaInteiro) e 'atestado' fazem setDoc merge em escalas/{rid}_{comp}.real[emp][dia]=statusEscala, igual à reclassificação; cancelarAjuste reverte se !diaFechado e real===statusEscala. 3) Linha.statusPrevisto (só prevista) + renderPraticado(statusPraticado). Falta: fundir de vez os 2 modais (Editar marcações × Reorganizar) — depende de decisão app-only vs Sólides." },
