...
const SEED: Seed[] = [
  ...
  // ── PTRP / afastamentos ──
  { modulo: "gorjetas", resp: "ia", titulo: "FIX: freela mensalista travava a divisão da gorjeta o mês inteiro (empregadoAtivoEm=true todo dia, sem status fechável → nenhum dia completa → fechadoAteDia=0). Agora freela só bloqueia nos dias em que TEM status (marcado); dias sem marcação não travam.", descricao: "core/escala/fechamentoPraticada.ts: freelaMensalista && !temStatus(real||prevista) → continue. Pra incluir a freela na gorjeta: marcar os dias trabalhados como FR na Escala Praticada → fechar no grid do PTRP (statusFechavel lê real??prevista='freela' → fecharGrid grava real+solides_sync). Caveat: se esquecer de marcar um dia trabalhado, ela sai da divisão daquele dia (não bate ponto, sistema não sabe). Possível melhoria: marcar FR direto no grid do PTRP + aviso 'freela sem marcação'." },
