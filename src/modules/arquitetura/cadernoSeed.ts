...
const SEED: Seed[] = [
  ...
  // ── Compras / Contagens ──
  { modulo: "compras", resp: "gustavo", titulo: "Módulo Solicitação de Compras — reforma grande EM ANDAMENTO. FEITO: pedido mínimo estruturado; motor sugestao.ts; SugestoesTab 3 colunas + itens OK adicionáveis; tela ÚNICA de Insumos (InsumosManager) espelhada em Contagens e Compras + 'Definir estoque mínimo' em lote + agrupar por produto/fornecedor; IA monta a sugestão (api/compras-ia.ts + botão 'Montar com IA' na SugestoesTab, persiste em comprasSugestaoIA/{rid}, resumo por fornecedor, editável, 'restaurar IA'). FALTA: (3) WhatsApp copiar + wa.me (app do celular); (4) vínculo Recebimento↔Pedido (IA cruza NF×pedido, baixa, divergências, retroativo nos 2 sentidos — recebimentoNotaId↔pedidoVinculadoId já nos tipos); auto-disparo da IA ao concluir a contagem (hoje é botão manual); (5) IA mesclar duplicados (sugerir grupos).", descricao: "IA compras: /api/compras-ia respeita repor-até-mínimo, múltiplo do pacote (fator), pedido mínimo por item (minPedido) e valor mínimo do pedido (pedidoMinimoValor, sobe itens pra bater). Resultado salvo em comprasSugestaoIA/{rid} (não re-avalia ao abrir; 'Refazer com IA' após nova contagem). baseQtd = IA ?? regra; edição sobrepõe; 'restaurar IA' limpa ajustes do fornecedor. Padrão IA: api/*.ts Anthropic direto (ANTHROPIC_API_KEY + idToken Firebase). Insumo = coleção 'insumos'." },
