...
const SEED: Seed[] = [
  ...
  { modulo: "geral", resp: "ia", titulo: "Reorg de categorias/módulos + ícones lucide (Fases 1 e 2) FEITA: <ModuleIcon> central (emoji→lucide) no registro + chrome das telas (títulos/abas), categoria MINHAS INFORMAÇÕES, Financeiro fundido em Administrativo, MASTER com Governança/Caderno/Propostas, selos Beta/Em dev removidos.", descricao: "Portal do Empregado quebrado em módulos (deep-link), Perfis em Configurações (não-master), Dados da empresa/Módulos viram links dedicados. Emojis decorativos inline (fora do chrome) mantidos por decisão." },
  { modulo: "configuracoes", resp: "ia", titulo: "Perfis de Acesso ainda usa gate isMaster||perfisAcesso.ver via link fixo (não é módulo ativável no grid). Se quiser, virar módulo de verdade no registro + activation." },
  ...
  // ── Compras / Contagens ──
  { modulo: "compras", resp: "gustavo", titulo: "Módulo Solicitação de Compras — reforma grande. FEITO: pedido mínimo estruturado; motor sugestao.ts; tela ÚNICA de Insumos (InsumosManager) em Contagens+Compras + 'Definir estoque mínimo' em lote + agrupar produto/fornecedor; 'Novo pedido' unificado (origem CONTAGEM — lista sessões não consumidas via Pedido.origemContagemSessaoId — ou AVULSO por fornecedor/categoria, todos/selecionar) → builder 3 colunas + Montar com IA; WhatsApp copiar + wa.me; IA mesclar duplicados (api/insumos-duplicados-ia); baixa: no 'Receber' do pedido dá pra VINCULAR a NF do Recebimento (sugere por fornecedor, casa itens NF×pedido, preenche qtd, divergência, link mão dupla recebimentoNotaId↔pedidoVinculadoId). FALTA: botão 'cruzar com pedido' no LADO do Recebimento (RecebimentoPage, 2173 linhas — não mexido); IA de match NF×pedido (hoje é match por nome/fornecedor); auto-disparo da IA ao concluir contagem (hoje botão).", descricao: "Endpoints: /api/compras-ia (sugestão por fornecedor, respeita mínimo/pacote/minPedido/pedidoMinimoValor, persiste em comprasSugestaoIA/{rid}), /api/insumos-duplicados-ia (grupos de duplicados). Novo pedido = NovoPedidoModal; aba Sugestões removida. Baixa no ReceberModal (PedidosTab) — precisa recebimentos+insumos passados pela ComprasPage. Padrão IA: api/*.ts Anthropic direto (ANTHROPIC_API_KEY + idToken). Insumo = coleção 'insumos'." },
  ...
  { modulo: "contagens", resp: "ia", titulo: "Lançar contagem: agrupamento por categoria OU fornecedor prioritário, steppers, desktop com rail de progresso." },
  { modulo: "compras", resp: "ia", titulo: "FEITO: QtyStepper (−/+ com número digitável no meio, h-9, mobile+desktop) em Editar pedido e no builder do Novo pedido. Total estimado saiu da mensagem do fornecedor (WhatsApp/copiar) e virou box interno na tela de enviar. Unidade faltando NÃO trava: ao abrir Editar, a IA (api/insumo-unidade-ia) atribui unidade aos itens sem unidade e mostra banner; item sem unidade vira select âmbar; salvar grava a unidade escolhida no cadastro do insumo (só quando o insumo estava sem).", descricao: "QtyStepper exportado de PedidosTab e reusado no NovoPedidoModal (step = fatorCompra quando >1). precisaUnidade(u)=!u. api/insumo-unidade-ia infere unidade de compra pelo nome (ignora peso/volume da embalagem), retorna dentro de UNIDADES_LISTA." },
  { modulo: "recebimento", resp: "ia", titulo: "FEITO: vínculo NF ⇄ pedido PELO LADO DO RECEBIMENTO (VincularPedidoBlock no DetalheModal). Camada 1 (código, matchPedido.ts/montarCandidatos) ranqueia os pedidos abertos (enviado/aprovado, não vinculados) por CNPJ/nome do fornecedor + janela de tempo (pedido nos ~15d antes da emissão) + itens em comum + proximidade de valor; 'sugestão forte' = CNPJ bate + único aberto do fornecedor. Camada 2 (IA, api/recebimento-match-ia) confere item a item (ok/qtd_menor/qtd_maior/faltou/extra) casando por similaridade de nome e sugere status. Resultado CACHEADO em RecebimentoNota.matchIA (não re-roda; só ao trocar o pedido-alvo ou 'Reanalisar'). Vincular = baixa mão-dupla (pedido.recebimentoNotaId+status+qtdRecebida por item / nota.pedidoVinculadoId+conforme+divergencia); desvincular volta o pedido a 'enviado'.", descricao: "Só doc-de-mercadoria (esconde em conta_fixa). Sem itens lidos na NF → deixa vincular mas avisa que não confere item a item (reprocessar OCR). Auto-dispara a IA 1x por pedido-alvo (analisadoParaRef). Coleções: pedidos + recebimentos (rules já existem). Status item: matchIA.itens[].status. FALTA: eventual botão de vínculo direto na tabela sem abrir o detalhe." },
  { modulo: "contagens", resp: "ia", titulo: "FEITO: contagem colaborativa AO VIVO + ciclo de vida completo. Sessão viva (contagemSessoes/{rid}_{data}_{turno}, status em_andamento) sincroniza por onSnapshot; vários contam junto, salva por campo (protegido por editandoRef). Finalizar cria registro status 'realizada' (id vira sessaoId das contagens) e apaga a viva. Abortar (dupla confirmação) → registro 'cancelada' (retomável). Abas novas: Lançar · Painel (dashboard: contados hoje/abaixo do mínimo/sem contagem/reposição R$ + top faltas + link Compras) · Histórico (sessões com status colorido, clicáveis → detalhe; canetinha edita se permissão 'editarFinalizada' e não travada; cancelada→retomar) · Insumos. Removida a 'Visão atual'.", descricao: "Status: em_andamento→realizada→editada→pedido(TRAVA)/cancelada. Editar re-materializa as contagens da sessão (apaga por sessaoId, regrava) + status editada. Trava por pedido: NovoPedidoModal marca contagemSessoes.status='pedido'+pedidoId ao gerar pedido a partir dela (anti-duplicidade; só o pedido edita depois). Permissão nova no actionCatalog: contagens.editarFinalizada. data/turno subidos pra ContagensPage (Histórico 'Continuar/Retomar' seta e volta pro Lançar). Regra Firestore contagemSessoes já deployada." },
  ...
  // ── PTRP / afastamentos ──
  { modulo: "gorjetas", resp: "ia", titulo: "FIX: freela mensalista travava a divisão da gorjeta o mês inteiro (empregadoAtivoEm=true todo dia, sem status fechável → fechadoAteDia=0). Agora freela só bloqueia nos dias em que TEM status; dias sem marcação não travam.", descricao: "core/escala/fechamentoPraticada.ts: freelaMensalista && !temStatus(real||prevista) → continue." },
  { modulo: "ptrp", resp: "ia", titulo: "FEITO: grid 'Fechar praticada' entende a prevista DERIVADA do cadastro dos freelas mensalistas (derivedScheduleForEmpregado) — mostra o status sugerido (TR/FO, badge violeta 'F' + tracejado 'sugerido'), e deixa CONFIRMAR (🔒 Fechar) sem ir na Escala nem tocar na Sólides. Botão 'semana' por empregado confirma a semana toda de uma vez.", descricao: "statusFechavel: real → prevista → derivada(freela). Freela mensalista recebe gorjeta com status 'trabalho' (não 'freela' — STATUS_RECEBE.freela=false; FR é do freela por turno/freelasDoDia). calcularDivisaoDia já cai na derivada, então confirmar grava real='trabalho'+solides_sync e vira a fonte auditável. Caveat: se o cadastro/horário do freela estiver errado num dia, o sugerido vem errado — conferir antes de confirmar." },
  ...
  { modulo: "ptrp", resp: "ia", titulo: "DECIDIDO: o gatilho pra subir pra escala praticada é o FECHAMENTO do dia ('Fechar dias' grava real + realAjustes.solides_sync). Abono/atestado NÃO grava escalas.real na hora — só trata o dia (vira statusPraticado); a visibilidade imediata é a coluna 'Praticado' da Conferência. Sem backfill separado: fechar o dia sobe.", descricao: "Wiring conferido OK (rid, doc id AAAA-MM, empId, dia, célula = real ?? prevista). Removida a gravação instantânea do abono em escalas.real (desviava do modelo do usuário e mostrava tracejado antes de fechar). Casos antigos (ex.: Sérgio): fechar o dia dele no PTRP → sobe como praticada sólida." },
  { modulo: "ptrp", resp: "ia", titulo: "FEITO: Reorganizar batidas reabre na ordem SALVA (não re-ordena cronológico); Conferência ganhou coluna Previsto × Praticado (badge + '•' quando difere).", descricao: "1) abrirReorganizar recebe reorgExistente e achata os pares salvos. 2) Linha.statusPrevisto (só prevista) + renderPraticado(statusPraticado) — coluna nova na tabela e no card mobile. Falta: fundir de vez os 2 modais (Editar marcações × Reorganizar) — depende de decisão app-only vs Sólides." },
  ...
  // ── Ponto (PTRP) ──
  { modulo: "ponto", resp: "gustavo", titulo: "Validar o LEIAUTE do AEJ (Portaria 671) contra o oficial + cadastrar CNPJ de cada empresa em restaurants.cnpj (o espelho/AEJ usam)", descricao: "AEJ (core/ptrp/aej.ts) sai com os DADOS completos mas larguras/ordem dos campos marcadas como PRELIMINAR. Espelho PDF e AEJ puxam empresaCnpj de activeRestaurant.cnpj — hoje fica em branco se não cadastrado." },
  { modulo: "ponto", resp: "gustavo", titulo: "Confirmar os shortcodes das 4 empresas no SOLIDES_TOKENS (Quibebe/Puba) e rodar 1 mês completo de sync p/ validar totais das 4", descricao: "Sync + Conferência validados no Lobozó (matriz). Falta garantir token/shortcode de todas e validar o fechamento das 4." },
  { modulo: "ponto", resp: "ia", titulo: "FEITO: fechamento mensal (Encerrar/Reabrir → ptrpApuracoes/ptrpFechamentos, trava a apuração) + Espelho de ponto PDF + AEJ. Próximo: neutralização/oficialização do Sólides (Fase 2) e REP-C físico (Fase 3)", descricao: "Encerrar mês congela snapshot por colaborador; UI trava tratamento/aprovação; Reabrir mantém o snapshot. Espelho e AEJ leem o snapshot ao vivo (ainda não do doc congelado)." },
];
