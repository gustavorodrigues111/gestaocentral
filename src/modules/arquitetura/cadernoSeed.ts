...
const SEED: Seed[] = [
  ...
  // ── Compras / Contagens ──
  { modulo: "compras", resp: "gustavo", titulo: "Módulo Solicitação de Compras — reforma grande EM ANDAMENTO. FEITO: pedido mínimo estruturado (Fornecedor.pedidoMinimoValor R$ + Insumo.minPedido qtd/item), motor sugestao.ts (contagem×mínimo, arredonda por pacote+minPedido, agrupa por fornecedor, aviso valor mínimo), SugestoesTab redesenhada (3 colunas Contagem|Estoque mínimo|Sugestão + itens OK opacos/adicionáveis). FALTA: (2) IA monta a sugestão ao ENVIAR a contagem (api/compras-ia.ts + ação 'concluir contagem' + persistir rascunho por fornecedor + espelho IA/restaurar); (3) WhatsApp copiar + wa.me (abre app do celular) no lugar/além do inbox interno; (4) vínculo Recebimento↔Pedido: IA cruza NF com pedidos abertos ('é o pedido do dia tal?'), avalia divergências, dá baixa; vínculo manual + retroativo nos 2 sentidos (recebimentoNotaId↔pedidoVinculadoId já nos tipos).", descricao: "Decisões do usuário: 3 colunas = Contagem|Estoque mínimo|Sugestão (pedido mínimo embutido no cálculo). IA monta tudo de saída MAS só 1x quando a contagem é enviada (persiste, não re-avalia ao abrir; guarda espelho p/ restaurar + versão editada). Pedido fica em Compras 'aguardando recebimento', espelhado no Recebimento. Padrão IA do projeto: api/*.ts chamando Anthropic direto (ANTHROPIC_API_KEY, requireUser via Firebase idToken); ver api/contagens-ia.ts e api/ocr-nota.ts. Insumo é a coleção 'insumos' (não ftInsumos)." },
