...
import { Rewind, CalendarDays, MapPin } from "lucide-react";
  ...
  return (
    <div className="space-y-4 max-w-2xl">
      <div className="rounded-lg border border-amber-200 dark:border-amber-800 bg-amber-50/60 dark:bg-amber-900/15 p-3 text-sm text-gray-700 dark:text-gray-200">
        <Rewind size={14} className="inline align-[-2px] mr-1" /><strong>Turnos passados</strong> — lance um turno de freela numa data anterior já com
        entrada, saída e intervalos. Cai direto no Fechamento pra precificação. Acesso exclusivo
        (permissão concedida à parte).
      </div>

      <Button onClick={() => setShowModal(true)}><span className="inline-flex items-center gap-1.5"><Rewind size={15} /> Lançar turno passado</span></Button>
            ...
            {recentes.map((s) => {
              ...
              return (
                <div key={s.id} className="flex items-center gap-3 flex-wrap rounded-lg border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 px-3 py-2 text-sm">
                  ...
                  <span className="text-gray-500 dark:text-gray-400 inline-flex items-center gap-1"><CalendarDays size={12} /> {fmtDia(s.date)}</span>
                  ...
                  {s.area && <span className="text-gray-500 dark:text-gray-400 inline-flex items-center gap-1"><MapPin size={12} /> {s.area}</span>}
