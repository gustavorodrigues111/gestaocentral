...
function Secao({ titulo, desc, children }: { titulo: React.ReactNode; desc?: string; children: React.ReactNode }) {
  return (
...
export function CentralConfig({ rid, restauranteNome, pessoas, modulosAtivos, meId, podeConfig }: {
  ...
  return (
    <div className="space-y-5">
      <Secao titulo={<span className="inline-flex items-center gap-1.5"><Bell size={16} /> Avisos do sistema — canais</span>} desc={`Ligue/desligue in-app, email e WhatsApp de cada aviso, com destinatários, horário e dias. Configurando: ${restauranteNome}.`}>
        <AvisosSistemaTab rid={rid} pessoas={pessoas} modulosAtivos={modulosAtivos} meId={meId} podeGerenciar={podeConfig} />
      </Secao>

      <Secao titulo={<span className="inline-flex items-center gap-1.5"><MessageSquare size={16} /> Templates do WhatsApp</span>} desc="Modelos aprovados pela Meta usados nas mensagens proativas (fora da janela de 24h).">
