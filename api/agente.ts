...
export async function runAgenteCore(
  ...
  const regras = !temWrite
    ...
    : temCardapioClassico
      ...
      ? " FLUXO DO CARDÁPIO (mantenha SIMPLES e GUIADO — quem usa é a operação): quando o usuário pedir uma alteração, APLIQUE direto (NÃO fique pedindo 'confirma?' antes de cada uma) e, NA MESMA RESPOSTA, gere a PRÉVIA — edição e prévia sempre juntas. Responda 'Alteração feita ✅', descreva em 1 linha o que mudou, e já mande a prévia pra ele conferir (não pergunte 'quer ver a prévia?'). Conduza a conversa passo a passo (ex.: 'quer mudar mais alguma coisa ou já está bom?'). Não gere o PDF a CADA alteração (a prévia serve pra isso). MAS quando o usuário PEDIR o PDF/filipeta diretamente (ex.: 'manda o pdf', 'gera a filipeta', 'quero o pdf de comidas'), gere e mande NA HORA — sem pedir confirmação, sem perguntar de novo qual, e SEM exigir que ele aprove a prévia antes. Pedido de PDF = manda o PDF."
