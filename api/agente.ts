...
const READ_TOOLS: Record<string, { desc: string; cols: string[] }> = {
  ...
  ler_vendas_produto:   { desc: "RELATÓRIO OFICIAL 'Vendas por Produto' do PDV (Altec), por MÊS — a fonte COMPLETA e correta de quanto cada produto vendeu (caixa encerrado). Use esta pra perguntas do tipo 'quantas X venderam no mês', 'produto mais vendido', 'faturamento por categoria'. Cada doc é um mês (campo `competencia` YYYY-MM) e tem: totalProdutos, totalQtd, totalFatBruto, totalFatLiquido e produtos[{id, produto, categoria, qtd, fatBruto, fatLiquido, pctTotal, curva (A/B/C)}] com TODOS os produtos do mês. Filtre por restaurantId e periodo (YYYY-MM). IMPORTANTE: prefira esta ferramenta em vez do rankProdutos de ler_vendas_altec — aquele é só o top-10 do dia e SUBCONTA o mês. Valores em reais; qtd em unidades.", cols: ["vendasProdutoAltec"] },
...
const SKILL_TOOLS: Record<string, SkillTool> = {
  ...
  gerar_pdf: {
    desc: "Gera o PDF FINAL da filipeta do Puba e devolve o link. `cardapio` diz QUAL folha: 'comidas' | 'bebidas' | 'vinhos' (Carta de Vinhos) | 'especiais_almoco' (Especiais de Almoço) | 'especiais' (Especiais do dia) | 'dobravel' (CARDÁPIO DOBRÁVEL: 1 folha A4 dobrada ao meio — frente = Bebidas na contracapa + capa 'Comidas e Bebidas'; verso = Comidas ocupando 1,5 coluna + Especiais de Almoço no rodapé da direita) | 'todos'. REGRA FIXA: quando o usuário pedir 'comidas e bebidas' / 'o cardápio' / 'o cardápio principal' / 'a filipeta' / 'manda o pdf de comidas e bebidas', use SEMPRE 'dobravel' (peça dobrável de 2 páginas, o padrão) — NÃO pergunte 'dobrável ou separadas' e NUNCA gere comidas e bebidas em folhas separadas. Só use 'comidas' ou 'bebidas' sozinhos se pedir EXPLICITAMENTE 'só comidas'/'apenas bebidas'. Pergunte qual folha apenas se o usuário NÃO disser nada sobre qual cardápio quer.",
    tipo: "write",
...
export async function runAgenteCore(
  ...
  const regras = !temWrite
    ...
    : ehCardapioSite
      ? " FLUXO DO CARDÁPIO (CONFIRME antes de aplicar). IMPORTANTE: cada restaurante tem a SUA estrutura — NÃO assuma peças/seções de outro restaurante (nada de 'dobrável', 'Especiais de Almoço', 'filipeta' etc., que são de OUTRO cardápio). (1) SEMPRE use ler_cardapio_site ANTES de qualquer coisa pra ver os cardápios, seções e pratos REAIS deste restaurante, com nome/preço. (2) ANTES de aplicar, mande UMA mensagem curta CONFIRMANDO o que SAI, o que ENTRA e em QUAL cardápio/seção (ex.: 'Vou tirar *X* e colocar *Y* no cardápio de *Comidas* — confirma?'). NUNCA aplique sem esse ok. (3) Se faltar info (preço, descrição, seção, ou em qual cardápio), PEÇA primeiro — não invente. (4) Só com o OK ('pode', 'confirma', 'isso', 'sim') chame aplicar_cardapio_site; depois responda 'Feito ✅' com 1 linha do que mudou e pergunte se quer ver a prévia atualizada (NÃO gere sozinho). (5) Na menor DÚVIDA (não achou o item, item parecido em mais de um cardápio, pedido ambíguo), PERGUNTE em vez de adivinhar. (6) Pra o PDF, gere UM cardápio por vez com gerar_pdf_site (o que o usuário pedir); se não disser qual, PERGUNTE."
    : temCardapioClassico
  ...
  const notaCardapio = !temCardapioClassico ? ""
    : ehCardapioSite
      ...
      : (canal === "whatsapp"
          ? " Aqui é WhatsApp: não há prévia visual na tela. Pra o usuário CONFERIR/APROVAR o cardápio (inclusive após uma alteração), chame gerar_previa — ela manda um link HTML que ele abre no celular. Quando pedirem o PDF/filipeta FINAL, chame gerar_pdf com o `cardapio` que ele pediu (comidas/bebidas/vinhos/especiais_almoco/especiais). REGRA FIXA: 'comidas e bebidas' / 'o cardápio' / 'a filipeta' / 'manda o pdf de comidas e bebidas' → SEMPRE o DOBRÁVEL (gerar_pdf cardapio='dobravel'), a peça padrão. NÃO pergunte 'dobrável ou separadas' e NUNCA gere Comidas e Bebidas em folhas separadas. Folha isolada só se pedirem EXPLICITAMENTE 'só comidas'/'apenas bebidas'. Pra cardápios distintos (vinhos, especiais), gere um por vez. Os links/arquivo aparecem sozinhos na conversa, não precisa colar a URL no texto. O arquivo chega LOGO DEPOIS da sua mensagem — então diga que ele 'segue abaixo 👇' (nunca 'em cima'), e NÃO afirme que já enviou/entregou: a entrega acontece após seu texto."
          : " Quando pedirem pra VER/MOSTRAR o cardápio ou a prévia, chame ler_cardapio: a prévia visual (HTML) aparece SOZINHA na tela — não precisa listar item por item, só confirme que está mostrando. Depois de aplicar uma alteração, a prévia atualizada também aparece sozinha. Quando pedirem o PDF / a filipeta / o arquivo final, chame gerar_pdf com o `cardapio` pedido (comidas/bebidas/vinhos/especiais_almoco/especiais). REGRA FIXA: 'comidas e bebidas' / 'o cardápio' / 'a filipeta' / 'manda o pdf de comidas e bebidas' → SEMPRE o DOBRÁVEL (gerar_pdf cardapio='dobravel'), a peça padrão (1 folha dobrada com Comidas e Bebidas juntos). NÃO pergunte 'dobrável ou separadas' e NUNCA gere Comidas e Bebidas em folhas separadas. Folha isolada só se pedirem EXPLICITAMENTE 'só comidas'/'apenas bebidas'. Pra cardápios distintos (vinhos, especiais do dia), gere um por vez. O link pra download aparece na conversa.");
  ...
  const notaAbertura = !temCardapioClassico ? ""
    : ` No COMEÇO da conversa, se o usuário não disser o que quer, pergunte QUAL cardápio ele quer editar — ${ehCardapioSite ? "Comidas, Bebidas ou Vinhos" : "o dobrável Comidas e Bebidas (seções Comidas / Bebidas / Especiais de Almoço), a Carta de Vinhos ou os Especiais do Dia"} (ou "todos") — e então vá DIRETO nas edições dessa folha. Se ele já chegar dizendo o que quer, pule a pergunta.`;
