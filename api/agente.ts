...
export async function runAgenteCore(
  ...
  const notaCardapio = !temCardapioClassico ? ""
    : ehCardapioSite
      ...
      : (canal === "whatsapp"
          ? " Aqui é WhatsApp: não há prévia visual na tela. Pra o usuário CONFERIR/APROVAR o cardápio (inclusive após uma alteração), chame gerar_previa — ela manda um link HTML que ele abre no celular. Quando pedirem o PDF/filipeta FINAL, chame gerar_pdf com o `cardapio` que ele pediu (comidas/bebidas/vinhos/especiais_almoco/especiais). REGRA FIXA: 'comidas e bebidas' / 'o cardápio' / 'a filipeta' / 'manda o pdf de comidas e bebidas' → SEMPRE o DOBRÁVEL (gerar_pdf cardapio='dobravel'), a peça padrão. NÃO pergunte 'dobrável ou separadas' e NUNCA gere Comidas e Bebidas em folhas separadas. Folha isolada só se pedirem EXPLICITAMENTE 'só comidas'/'apenas bebidas'. Pra cardápios distintos (vinhos, especiais), gere um por vez. Os links/arquivo aparecem sozinhos na conversa, não precisa colar a URL no texto."
