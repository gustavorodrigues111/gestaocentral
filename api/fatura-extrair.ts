...
const PROMPT =
  "Você recebe o PDF de uma FATURA de cartão de crédito (Itaú, Santander, etc). Extraia TODOS os lançamentos e os dados da fatura. Regras:\n" +
  ...
  "Responda SOMENTE um objeto JSON (sem texto antes/depois): { \"vencimento\": \"YYYY-MM-DD\"|null, \"totalFatura\": number|null, \"lancamentos\": [ { \"data\": \"DD/MM\", \"descricao\": \"...\", \"valor\": number, \"parcela\": \"XX/YY\"|null } ] }";
...
export default async function handler(req: VercelReq, res: VercelRes): Promise<void> {
  ...
  try {
    ...
    const parsed = JSON.parse(m[0]) as { vencimento?: string | null; totalFatura?: number | null; lancamentos?: Array<{ data?: string; descricao?: string; valor?: number; parcela?: string | null }> };
    ...
    res.status(200).json({ vencimento: parsed.vencimento || null, totalFatura: typeof parsed.totalFatura === "number" ? parsed.totalFatura : null, lancamentos });
  } catch (e) {
