...
function montarPrompt(cartoes: string[], empresaPropria: string, empresas: string[], categorias: string[], historico: HistItem[]): string {
  ...
  "1b) CRÍTICO — MÚLTIPLOS CARTÕES: a fatura pode ter VÁRIOS cartões (titular + adicionais), cada um com um cabeçalho 'final XXXX' e um subtotal 'Lançamentos no cartão (final XXXX)'. Extraia os lançamentos de TODOS os cartões, inclusive os adicionais — não pare no primeiro. Cartões adicionais costumam vir numa GRADE COMPACTA de várias colunas onde a 'descrição' é só uma categoria/cidade genérica (ex: 'DIVERSOS .SAO PAULO', 'MORADIA .JUNDIAI', 'VESTUARIO .Sao Paulo'), com a data numa coluna e o valor noutra. Inclua ESSAS linhas também: se não houver nome de loja, use a categoria+cidade como descricao. Não descarte um lançamento só porque ele não tem nome de estabelecimento claro.\n" +
  "1c) AUTOCHECK OBRIGATÓRIO antes de responder: some os valores de TODOS os lançamentos que você vai retornar (contando estornos como negativos, e EXCLUINDO o cronograma de parcelas futuras da regra 4c). Essa soma tem que bater com o totalFatura (item 6). Se não bater, você PULOU lançamentos — reescaneie a fatura, principalmente os cartões adicionais e as grades multi-coluna, até fechar. Confira também os subtotais por cartão ('Lançamentos no cartão (final XXXX)') e por seção ('Total lançamentos inter.', 'Lançamentos produtos e serviços', 'Total de outros lançamentos'): a soma dos seus lançamentos de cada bloco deve bater com o subtotal impresso do bloco.\n" +
