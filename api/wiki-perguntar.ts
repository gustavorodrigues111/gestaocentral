...
function montarPrompt(pergunta: string, processos: ProcIn[]): string {
...
export default async function handler(req: VercelReq, res: VercelRes): Promise<void> {
  ...
  const body = (typeof req.body === "string" ? safeParse(req.body) : req.body) as { pergunta?: string; processos?: ProcIn[] } | null;
  ...
  const processos = (Array.isArray(body?.processos) ? body!.processos : [])
    ...
    .slice(0, 120);
  if (processos.length === 0) { res.status(200).json({ resposta: "Ainda não há nenhum processo documentado nesta empresa. Documente o primeiro processo pra que eu possa responder a partir dele.", fontesIds: [] }); return; }
  ...
  try {
    const payload = {
      ...
      messages: [{ role: "user", content: [{ type: "text", text: montarPrompt(pergunta, processos) }] }],
    };
    ...
    const parsed = JSON.parse(m[0]) as { resposta?: string; fontesIds?: unknown };
    ...
    const fontesIds = Array.isArray(parsed.fontesIds)
      ...
      : [];
    res.status(200).json({ resposta: String(parsed.resposta || "").trim() || "Não consegui formular uma resposta.", fontesIds });
  } catch (e) {
